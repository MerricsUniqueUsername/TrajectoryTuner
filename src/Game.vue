<template>
  ...
  <div class="game">
    ...
    <div class="username">
    {{ this.username }} | <a href="#" @click="logout()">Log Out</a>
  </div>
    <!-- Graph -->
    <Graph ref="graph" />
...
export default {
  ...
  methods: {
      ...
      const cookie = Cookies.get('token');
        ...
        let results = await fetch(backendURL + "/graphql",{
          ...
          body: JSON.stringify({
            query: `
            mutation {
              submitSolution(jwt: "${cookie}", level: ${this.$route.params.id}, expression: "${this.expression}")
            }`
          })
    ...
    async getUsername() {
      ...
      try {
        ...
        let data_json = await results.json();
        this.username = data_json.data.getUser.username;
        this.level_on = data_json.data.getUser.level_on;
  ...
  async mounted() {
    await this.getUsername();
    if(this.level_on < this.$route.params.id) {
      this.$refs.graph.setLevel(this.level_on);
      this.router.push('/level/' + this.level_on,  { replace: true });
    }
    else {
      await this.$refs.graph.setLevel(this.$route.params.id);
    }
  },
